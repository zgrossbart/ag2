 ...
 ******************************************************************************/
"use strict";
var gromit_ts_1 = require('./gromit.ts');
var gromit_oauth;
(function (gromit_oauth) {
    var reqs = [];
    var oauthCallback;
    var oauthstate;
    var Params = (function () {
    ...
    */
    ...
    gromit_oauth.doRequest = doRequest;
    /**
     * @private
     *
    ...
    function request(req) {
        if (gromit_ts_1.gromit.getToken()) {
            req.headers.Authorization = gromit_ts_1.gromit.getTokenType() + ' ' + gromit_ts_1.gromit.getToken();
        }
        if (!req.errorCallback) {
            ...
            */
            req.errorCallback = function (code, subcode, reason) {
                gromit_ts_1.gromit.println('errorCallback(' + code + ', ' + subcode + ', ' + reason + ')');
                if (subcode === 'NotFound') {
                    gromit_ts_1.gromit.showFatalError(gromit_ts_1.gromit.i18n.getI18n_general_error_notfound(req.url));
                }
                else {
                    gromit_ts_1.gromit.showGeneralError(code, subcode, reason);
                }
            };
        }
            ...
            */
            ...
            setTimeout(function () {
                ...
                if (clientTime > serverTime) {
                    $('div.invalidTime').children('h1').text(gromit_ts_1.gromit.i18n.invalid_client_time_ahead);
                }
                else {
                    $('div.invalidTime').children('h1').text(gromit_ts_1.gromit.i18n.invalid_client_time_behind);
                }
                $('div.invalidTime').children('p').text(gromit_ts_1.gromit.i18n.getI18n_invalid_client_time_error(gromit_ts_1.gromit.fullDateTimeFormat(clientTime)));
            }, 3000);
            return false;
        };
        var handle401 = function (req) {
            if (!validateTime()) {
                gromit_ts_1.gromit.println('the time was not valid');
                return;
            }
            /*
            ...
            */
            gromit_ts_1.gromit.clearToken();
            gromit_ts_1.gromit.clearTokenType();
            reqs.push(req);
            if (reqs.length === 1) {
                doLogin(req);
            }
        ...
        responsePromise.error(function (data, status, headers, config) {
            if (status === 0) {
                ...
                */
                gromit_ts_1.gromit.showFatalError('Unable to contact server at ' + req.url);
            }
                ...
                */
                if (status === 404) {
                    ...
                    */
                    ...
                    if (data) {
                        gromit_ts_1.gromit.showFatalError(gromit_ts_1.gromit.i18n.getI18n_fatal_request_error(req.url, status, data));
                    }
                    else {
                        gromit_ts_1.gromit.showFatalError(gromit_ts_1.gromit.i18n.getI18n_fatal_request_error(req.url, status, ''));
                    }
                    if (window.console) {
                        console.error(gromit_ts_1.gromit.showFatalError(gromit_ts_1.gromit.i18n.getI18n_fatal_request_error(req.url, status, data)));
                    }
                }
            }
        });
    ...
    gromit_oauth.request = request;
    /**
     * Get the current authentication token
     */
    function getAuthToken() {
        return gromit_ts_1.gromit.getTokenType() + ' ' + gromit_ts_1.gromit.getToken();
    }
    gromit_oauth.getAuthToken = getAuthToken;
    ;
    /**
     * @private
     */
    ...
    gromit_oauth.requestPromise = requestPromise;
    ;
    /**
     * @private
     *
    ...
    function doLogin(req) {
        if (gromit_ts_1.gromit.isEmpty(gromit_ts_1.gromit.AuthUrl)) {
            throw 'Unable to log in without gromit.AuthUrl';
        }
        oAuthAuthenticate(gromit_ts_1.gromit.AuthUrl, function (/*String*/ token, /*String*/ type) {
            gromit_ts_1.gromit.setToken(token);
            $.cookie('gromitTokenCookie', token);
            gromit_ts_1.gromit.setTokenType(type);
    ...
    function doLogout() {
        ...
        if (gromit_ts_1.gromit.isEmpty(gromit_ts_1.gromit.AuthLogoutUrl)) {
            throw 'Unable to log out without gromit.AuthLogoutUrl';
        }
        window.location.assign(gromit_ts_1.gromit.AuthLogoutUrl + '?target=' + redirectTo);
    }
    /**
     ...
     */
    function oAuthAuthenticateCompleteEventListener(event) {
     ...
     */
    function oAuthAuthenticateComplete(response) {
    ...
    ;
})(gromit_oauth = exports.gromit_oauth || (exports.gromit_oauth = {}));
